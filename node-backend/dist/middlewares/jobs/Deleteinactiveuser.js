...
const deleteAnonUser = async ()=>{
    ...
    await PromiseMap(users, async (user)=>{
        try {
            const animatedVideoPath = path.resolve(path.dirname(process.cwd() + '/') + `/uploads/animated-videos/${user.id}`);
            const audioPath = path.resolve(path.dirname(process.cwd() + '/') + `/uploads/podcasts/${user.id}/`);
            ...
            await AnimatedVideo.deleteMany({
                user_id: user.id
            });
