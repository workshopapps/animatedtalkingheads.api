...
worker.on('failed', async (job, err)=>{
    ...
    try {
        ...
        const testFolder = path.resolve(path.dirname(process.cwd() + '/') + `/pyhton-backend/data/user_data/${job.id}/`);
        fs.rmdir(testFolder, {
            recursive: true,
            force: true
        }, (error)=>{
        ...
        const animatedVid = await AnimatedVideo.findByIdAndUpdate(job.id, {
            video_url: process.env.reqHost + `/user_data/` + `${job.id}/animation_sound.mp4`,
            status: 'COMPLETED'
        });
...
const runPythonScript = async (jobConfig)=>{
    const res = await queue.add(jobConfig.animated_video_id, {
        jobConfig
    }, {
