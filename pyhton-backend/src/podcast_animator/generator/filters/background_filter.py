...
class BackgroundFilter:
    ...
    def bg_sq(
        self,
    ):  # This method stores a list containing the dynamic sequence of the background animation, inside the class variable 'self.bq_sq_files'
        self.bg_sq_files = []
        self.directory_path = self.directory_path / "animation"
        for files in self.directory_path.iterdir():
            self.bg_sq_files.append(files)

    def _compose_animation_schema(
        self,
    ):  # This method stores a dictionary of the format {frame_id: frame_image} in self.animation_frames
        self.animation_frames = {}
        counter = 0
        while counter < len(range(1, self.animation_frame_length + 1)):
            if (
                counter % self.interval == 0
            ):  # Checks for the interval to load in the dynamic sequence
                ...
                self.animation_frames.update(
                    {str(counter): self.directory_path.parent / "default.png"}
                )
                counter += 1

    def add_to_canvas(
        self, frame_data: tuple[int | img_obj]
    ) -> img_obj:  # This method adds it to the canvas and returns the canvas as a filtered object
